...
var comentarios=[];
var estrellas;
//defino esta variable donde voy a cargar todos los productos para luego matchearlos con los productos relacionados
...
//Traemos la lista de productos y la cargamos en la variable todosLosProductos
getJSONData(PRODUCTS_URL).then(function(resultObj){
    if (resultObj.status === "ok")
    {
         //Muestro la informacion
         todosLosProductos=resultObj.data;
         //Llamo a este metodo para mostrar la informacion de los productos relacionados
        showProductsrelatedList()
    }
});
...
//la aprte del carousel la hice hardcodeada por que se la cantidad de imagenes, pero podria hacerse con un for para saber cuantas son.
...
//Para finalizar le asigno el comentario viejo a la variable nuevohtml, luego traigo las estrellas y el resto de la informacion 
...
//Esta funcion recorre la lista de productos(todosLosProductos previamente cargada)  y la lista de productos relacionados del producto seleccionado(producto.relatedProducts)
//Chequea si la posicion del producto dentro de la lista de productos es la misma que la posicion del producto relacionado
//Ejemplo en productos relacionados del onix tiene 1 y 3, por lo tanto busco esas posiciones en la lista de productos
//una vez encontro dicho producto lo muestro
function showProductsrelatedList(){
   
    let htmlContentToAppend = "";
    for(let i = 0; i < todosLosProductos.length; i++){
        for(let x = 0; x < producto.relatedProducts.length; x++){
             if(i===producto.relatedProducts[x]){
                let productos = todosLosProductos[i];
                htmlContentToAppend += `
        <div class="list-group-item list-group-item-action">
        <a href="product-info.html">
            <div class="row">
                <div class="col-3">
                    <img src="` + productos.imgSrc + `" alt="Img${productos.name}" class="img-thumbnail">
                ...
                <div class="col">
                    ...
                    <div class="d-flex w-100 justify-content-between">
                         ...
                         <small class="text-muted"><b>Costo: ` + productos.cost + `  ` + productos.currency + `</b></small>
                   </div>
               </div>
            </div>
            </a>
    ...
    //Luego de terminar el recorrido de los elementos y salir del for busco el elemento prodrela-list..... en el Html 
